...
export const config = {
  ...
  firebase: {
    apiKey: process.env.APP_FIREBASE_API_KEY || 
      (functions.config().firebase && functions.config().firebase.api_key) || "",
    authDomain: process.env.APP_FIREBASE_AUTH_DOMAIN || 
      (functions.config().firebase && functions.config().firebase.auth_domain) || "",
    projectId: process.env.APP_FIREBASE_PROJECT_ID || 
      (functions.config().firebase && functions.config().firebase.project_id) || "",
    storageBucket: process.env.APP_FIREBASE_STORAGE_BUCKET || 
      (functions.config().firebase && functions.config().firebase.storage_bucket) || "",
    messagingSenderId: process.env.APP_FIREBASE_MESSAGING_SENDER_ID || 
      (functions.config().firebase && functions.config().firebase.messaging_sender_id) || "",
    appId: process.env.APP_FIREBASE_APP_ID || 
      (functions.config().firebase && functions.config().firebase.app_id) || "",
  },
};
